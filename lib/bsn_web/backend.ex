defmodule BsnWeb.Backend do
  ...
  def get(%{"query" => "Trips"}, %{id: id}, context) do
    ...
    """
    trip = Enum.at(Neo4j.query!(Neo4j.conn, cypher),0)

    stops = get(%{"query" => "Stops"}, %{trip_id: id}, context)

    Map.merge(trip, %{"stops" => stops})
  end

  def get(%{"query" => "Stops"}, %{trip_id: trip_id}, _context) do
    cypher="""
